...
1 元 = 100 credits = 100 fen (CNY's smallest unit). This invariant means
`amount_total` returned by Stripe (in fen) maps 1:1 to credits — `topup`
trusts the metadata.credits field with the amount as fallback.
...
`STRIPE_PAYMENT_METHODS`). The Stripe SDK itself is imported lazily
inside `build_checkout_session` / `verify_webhook` so the rest of the
codebase loads even when `stripe` isn't installed.
...
has not enabled it makes Session.create return 4xx. The list is echoed
by `/v1/topup_packs` so the frontend can render the right hint.
...
| `POST /webhooks/stripe` | Stripe-Signature header | Verifies signature, parses `checkout.session.completed` **and** `checkout.session.async_payment_succeeded`, calls `service.topup(user_id, credits, request_id="stripe:<session_id>")`. Idempotent — duplicate sessions return `duplicate=true` instead of crediting twice. |

Pack catalog (`config/stripe_packs.json`) holds `{amount_yuan,
amount_fen, credits, label, description}` per pack. `stripe_gateway.resolve_amount`
also accepts `custom_yuan` for arbitrary amounts inside `min_custom_yuan`–`max_custom_yuan`.

The `stripe_gateway` JSON-serializes the verified Stripe event before
returning (via `to_dict_recursive` → `to_dict` → `json.dumps` fallback)
so callers never depend on the SDK's runtime types.
