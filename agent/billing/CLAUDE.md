...
`STRIPE_SUCCESS_URL`, `STRIPE_CANCEL_URL`). The Stripe SDK itself is
imported lazily inside `build_checkout_session` / `verify_webhook` so
the rest of the codebase loads even when `stripe` isn't installed.
