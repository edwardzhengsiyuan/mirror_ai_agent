...
main() {
  ...
  require_var APP_SECRET_KEY  # signs dashboard session cookies — must be stable
...
mkdir -p \"${DEPLOY_PATH}/storage\""
  ...
  # Local checks hit 127.0.0.1:8000 directly (bypass Caddy), so we check the
  # routes Flask actually serves. /openapi.json and /_swagger are still served
  # locally for internal probes; Caddy blocks them from the public.
  ...
  if curl -fsS http://127.0.0.1:8000/health >/dev/null 2>&1; then
    ...
    if curl -fsS http://127.0.0.1:8000/docs >/dev/null 2>&1 \\
      ...
      # Sanity check: the legacy UIs should NOT be in the container image
      # (excluded via .dockerignore). Surface a non-fatal warning if they
...
exit 1" || die "local API did not become healthy on the server"
  ...
  if [[ "$DEPLOY_VERIFY_PUBLIC" == "1" ]]; then
    ...
    # Routes that MUST be reachable publicly (200).
    ...
    log "  public pages OK: / /login /register /dashboard /billing /docs"

    # Routes that MUST be blocked by Caddy (404). admin.html and app.html
    # should also be missing from the container image; openapi.json and
    # _swagger are served by Flask internally but Caddy hides them from
    # the public.
    local blocked_path
    ...
    log "  internal paths blocked: /app.html /admin.html /openapi.json /_swagger /admin/users"
  else
